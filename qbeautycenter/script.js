...
const ICONS = {
    ...
    phone: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" fill="#bc8797"/></svg>`,

    email: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" fill="#c39fcc"/></svg>`,

    website: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" fill="#bc8797"/></svg>`,

    tiktok: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" fill="#c39fcc"/></svg>`,
    ...
    contact: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm-9-2V7H4v3H1v2h3v3h2v-3h3v-2H6zm9 4c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" fill="#bc8797"/></svg>`,

    location: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="#bc8797"/></svg>`,
...
function initSparkles() {
    ...
    let particles = [];
    const colors = ['#c39fcc', '#bc8797', '#d4b5d8', '#d4a0b0', '#e8c7d3'];
...
function initializeLinks() {
    ...
    if (enabledLinks.length === 0) {
        linksContainer.innerHTML = '<p style="color: var(--text-muted); text-align: center; font-size: 13px;">No links available</p>';
    }
}
...
function initializeSocialLinks() {
    ...
    // Hide entire social section if none enabled
...
function generateVCard() {
    ...
    if (phone) vcard += `TEL;TYPE=CELL:${phone.replace(/\s+/g, '')}\n`;
...
function applyTheme() {
    ...
    if (CONFIG.theme.primaryColor) {
        ...
        const glow = CONFIG.theme.primaryGlowColor || lightenColor(CONFIG.theme.primaryColor, 20);
        root.style.setProperty('--primary-glow', glow);
        const rgb = hexToRgb(CONFIG.theme.primaryColor);
        if (rgb) {
            root.style.setProperty('--shadow-glow', `0 0 40px rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.2)`);
        }
    }
    ...
    if (CONFIG.theme.bgDark) root.style.setProperty('--bg-body', CONFIG.theme.bgDark);
    if (CONFIG.theme.bgCard) root.style.setProperty('--bg-card', CONFIG.theme.bgCard);
    if (CONFIG.theme.cardBorder) root.style.setProperty('--card-border', CONFIG.theme.cardBorder);
    if (CONFIG.theme.textPrimary) root.style.setProperty('--text-primary', CONFIG.theme.textPrimary);
    if (CONFIG.theme.textSecondary) root.style.setProperty('--text-secondary', CONFIG.theme.textSecondary);
